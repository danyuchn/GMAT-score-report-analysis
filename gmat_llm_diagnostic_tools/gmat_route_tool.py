...
description: Routes GMAT issues to appropriate training commands based on category and error code, with integrated command descriptions
version: 2.0.0
license: MIT
requirements: typing
"""
...
class Tools:
    def __init__(self):
        """Initialize the GMAT Issue Router tool with integrated command descriptions."""
        ...
        # Define the route table as a class attribute
    ...
    def route_gmat_issue(self, category: str, error_code: str) -> Dict[str, List[str]]:
        ...
        This function takes a GMAT category and error code, then returns a list of 
        recommended training commands to address the specific issue.
        
        :param category: GMAT category (CR / DS / GT / MSR / PS / RC / TPA)
        :param error_code: GMAT error code identifying the specific issue
        :return: Dictionary containing a list of recommended training commands
        """
        # Validate category
        ...
        # Get commands for the specific error code
        ...
        if not commands:
            ...
            return {
                "commands": [],
                "error": f"Error code '{error_code}' not found in category '{category}'. Available codes: {available_codes[:5]}..."  # Show first 5 to avoid overwhelming output
            }
    ...
    def get_command_description(self, command_name: str) -> Optional[str]:
        ...
        :return: Description of the command or None if not found
        """
        return self.command_descriptions.get(command_name)

    def get_commands_with_descriptions(self, category: str, error_code: str) -> Dict:
        """
        Get commands with their descriptions for a specific error code.
        
        :param category: GMAT category (CR / DS / GT / MSR / PS / RC / TPA)
        :param error_code: GMAT error code identifying the specific issue
        :return: Dictionary containing commands and their descriptions
        """
        ...
        for command in result["commands"]:
            ...
            commands_with_descriptions.append({
                "command": command,
                "description": description if description else "描述暫未提供"
            })
        
        return {
            "commands": result["commands"],
            "commands_with_descriptions": commands_with_descriptions
        }

    def get_available_categories(self) -> List[str]:
        """
        Get all available GMAT categories.
        
        :return: List of available GMAT categories
        """
        return list(self.route_table.keys())

    def get_error_codes_for_category(self, category: str) -> List[str]:
        """
        Get all available error codes for a specific GMAT category.
        
        :param category: GMAT category (CR / DS / GT / MSR / PS / RC / TPA)
        :return: List of available error codes for the category
        """
        if category not in self.route_table:
            return []
    ...
    def get_all_commands(self) -> List[str]:
        ...
        """
        return list(self.command_descriptions.keys())

    def get_error_codes_mapping(self, category: str = None) -> Dict:
        """
        Get error codes with Chinese descriptions for diagnosis mapping.
        
        :param category: Optional - specific GMAT category to get codes for
        :return: Dictionary mapping Chinese descriptions to error codes
        """
        error_mapping = {
            "CR": {
                # ERROR types
            ...
            "RC": {
                ...
                "選項相關性判斷困難": "RC_CHOICE_ANALYSIS_DIFFICULTY_RELEVANCE_JUDGEMENT",
        ...
        if category:
            return error_mapping.get(category, {})
