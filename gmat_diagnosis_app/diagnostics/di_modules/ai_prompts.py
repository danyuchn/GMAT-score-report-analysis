...
def generate_di_ai_tool_recommendations_legacy(df_di: pd.DataFrame) -> str:
    """
    使用原有方法生成建議 (作為備選方案)
    
    Args:
        df_di (pd.DataFrame): 包含DI科目診斷標籤的數據框
    ...
    # 收集所有診斷標籤
    all_tags = []
    for tags_list in df_di['diagnostic_params_list'].dropna():
    ...
    for zh_tag, count in tag_counts.items():
        ...
        # 檢查英文標籤是否與DI_TOOL_AI_RECOMMENDATIONS中的鍵匹配
        for recommendation_key in DI_TOOL_AI_RECOMMENDATIONS:
            if en_tag.upper() == recommendation_key or en_tag in recommendation_key or recommendation_key in en_tag.upper():
                matched_tools = DI_TOOL_AI_RECOMMENDATIONS[recommendation_key]
    ...
    for tag, tools, count in matched_tags:
        ...
        for tool in tools:
            if tool not in already_recommended:
                # 有些是直接工具名稱，有些前面有Tool:或AI Prompt:前綴
...
def generate_di_ai_tool_recommendations(df_di: pd.DataFrame) -> str:
    """
    根據DI科診斷標籤生成AI工具和提示建議
    使用新的診斷路由工具進行匹配
    ...
    try:
        ...
        # 如果新系統出現錯誤，回退到舊系統
        import logging
        logging.warning(f"New route tool failed for DI subject: {str(e)}, falling back to legacy method")
        return generate_di_ai_tool_recommendations_legacy(df_di) 