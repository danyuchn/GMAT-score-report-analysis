...
% This section seems redundant with the previous 'Required Input CSV Structure' and 'Overall Metrics', simplifying.
%\textbf{Common Inputs:}
%\begin{itemize}
 ...
 \item \textbf{Determine Time Pressure Status (\texttt{time\_pressure}):} This Boolean flag indicates potential time pressure during the test section. The flag is set to \texttt{True} if the overall time difference (\texttt{time\_diff}) is $\leq$ 3.0 minutes AND the time spent on any question within the final third of the section (\texttt{question\_time}) is less than 1.0 minute. Additionally, a user override option (\texttt{user\_override\_time\_pressure}) allows this flag to be set manually.
    ...
    \item \textbf{Identify Invalid Data (\texttt{is\_invalid}):} This flag identifies questions likely answered without genuine effort due to time constraints. The logic is as follows:
        \begin{itemize}
            ...
            \item \textbf{Scope:} Only checks questions in the final third (\texttt{question\_position} $>$ \texttt{Total Number of Questions} * 2/3). % CORRECTED: Added $ sign
            \item \textbf{Criteria Definition ("Abnormally Fast Response"):} A question (or its group for RC/MSR) is considered abnormally fast if it meets \textbf{at least one} of the following standards:
                \begin{itemize}
                    \item Absolute Time 1: \texttt{question\_time} $<$ 0.5 minutes. % CORRECTED: Added $ sign
                    \item Absolute Time 2: \texttt{question\_time} $<$ 1.0 minute. % CORRECTED: Added $ sign
                    \item Relative Single Question Time: \texttt{question\_time} $<$ (Average time for that question type in the first third * 0.5). % CORRECTED: Added $ sign
                    \item Relative Group Time (RC/MSR only): \texttt{group\_total\_time} $<$ (Average RC/MSR time in first third * Number of questions in group * 0.5). % CORRECTED: Added $ sign
                \end{itemize}
...
% ***** FIGURE: Chapter 1 Logic *****
    ...
    % CORRECTED: Added $ signs around > and <
    \node[param, above = 0.5cm of invalid_check, text width=5.5cm] (invalid_param) {Trigger: \texttt{time\_pressure}=True \& Pos $>$ Tot*2/3.\\ Criteria (OR): Q time $<$ 0.5min; $<$ 1.0min;\\ $<$ 50\% avg 1st 1/3 (single);\\ $<$ 50\% avg 1st 1/3 * N (RC/MSR group)}; % MODIFIED PARAM TEXT, explicit text width
...
\textbf{Implementation Context:} Boolean flags (\texttt{time\_pressure}, \texttt{is\_invalid}, \texttt{overtime}) are added as columns to the \texttt{pandas} DataFrame. Thresholds are assigned to variables based on conditional logic applied to the \texttt{time\_pressure} status. Filtering is achieved via DataFrame slicing (e.g., \texttt{df\_filtered = df[$\sim$df['is\_invalid']]}). % Note: ~ is math symbol, but ok in tt
            ...
            \begin{enumerate}
                ...
                \item Apply Annotations:
                    \begin{itemize}
                        ...
                        \newpage
                            \begin{itemize}
                                \item \textbf{Quant (Q):} If final \texttt{Z} $>$ 4.0 min. % CORRECTED: Added $ sign
                                % CORRECTED: Changed & to \&, added $ sign around >
...
% --- Start of the longtable with increased row spacing ---
% Remember to have \usepackage{longtable} in your preamble
{ % Start group to limit the scope of \arraystretch
\renewcommand{\arraystretch}{1.1} % Adjust 1.3 (e.g., to 1.2, 1.4, 1.5) to control row spacing

\begin{longtable}{|l|p{0.45\textwidth}|} % Your specified column widths
...
\textbf{Parameter Tag} & \textbf{English Description} \\ % Header content
...
\multicolumn{2}{|c|}{\tablename\ \thetable{} -- \textit{Continued from previous page}} \\ % Optional: Title for continued pages
\hline % Line below the continued title
\textbf{Parameter Tag} & \textbf{English Description} \\ % Header content for continuation pages
\hline % Line below header content
\endhead % Marks the end of the header for continuation pages

\hline % Line above the footer on pages before the last
\multicolumn{2}{|r|}{\textit{Continued on next page}} \\ % Footer content for pages before the last
...
% Note: The \\multicolumn lines for section headers remain the same
...
% CR/RC Reading/Comprehension/Interpretation errors will be listed under specific CR/RC sections below
\hline
...
% CR/RC Concept/Logic Application errors will be listed under specific CR/RC sections below
\hline
...
% RC Location errors will be listed under specific RC sections below
\hline
\multicolumn{2}{|c|}{\textbf{Section/Type Specific (MSR - DI Only)}} \\\\ % Changed header to be DI specific
\hline
...
\texttt{CR\_SPECIFIC\_QUESTION\_TYPE\_WEAKNESS\_NOTE\_TYPE} & CR Specific Question Type Weakness: (Type to be noted) \\\\
\multicolumn{2}{|c|}{\textit{CR Stem Understanding Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{CR\_STEM\_UNDERSTANDING\_DIFFICULTY\_VOCAB} & CR Stem Understanding Difficulty: Vocabulary challenges in stem \\\\
\texttt{CR\_STEM\_UNDERSTANDING\_DIFFICULTY\_SYNTAX} & CR Stem Understanding Difficulty: Syntactical complexity in stem causing slowdown \\\\
\texttt{CR\_STEM\_UNDERSTANDING\_DIFFICULTY\_LOGIC} & CR Stem Understanding Difficulty: Logical structure of stem hard to grasp \\\\
\texttt{CR\_STEM\_UNDERSTANDING\_DIFFICULTY\_DOMAIN} & CR Stem Understanding Difficulty: Domain-specific content in stem hard to grasp \\\\
\multicolumn{2}{|c|}{\textit{CR Reasoning Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{CR\_REASONING\_DIFFICULTY\_ABSTRACT\_LOGIC\_TERMINOLOGY\_UNDERSTANDING} & CR Reasoning Difficulty: Difficulty with abstract logic or terminology \\\\
\texttt{CR\_REASONING\_DIFFICULTY\_PREDICTION\_DIRECTION\_MISSING} & CR Reasoning Difficulty: Missing or slow prediction of answer direction \\\\
\texttt{CR\_REASONING\_DIFFICULTY\_CORE\_ISSUE\_IDENTIFICATION} & CR Reasoning Difficulty: Difficulty identifying the core issue of the argument \\\\
\texttt{CR\_REASONING\_DIFFICULTY\_CHOICE\_RELEVANCE\_JUDGEMENT} & CR Reasoning Difficulty: Difficulty judging answer choice relevance \\\\
\texttt{CR\_REASONING_DIFFICULTY\_STRONG\_DISTRACTOR\_CHOICE\_ANALYSIS} & CR Reasoning Difficulty: Difficulty analyzing strong distractor choices \\\\
\multicolumn{2}{|c|}{\textit{CR Choice Understanding Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{CR\_CHOICE\_UNDERSTANDING\_DIFFICULTY\_VOCAB} & CR Choice Understanding Difficulty: Vocabulary challenges in answer choices \\\\
\texttt{CR\_CHOICE\_UNDERSTANDING\_DIFFICULTY\_SYNTAX} & CR Choice Understanding Difficulty: Syntactical complexity in choices causing slowdown \\\\
\texttt{CR\_CHOICE\_UNDERSTANDING\_DIFFICULTY\_LOGIC} & CR Choice Understanding Difficulty: Logical structure of choices hard to grasp \\\\
\texttt{CR\_CHOICE\_UNDERSTANDING\_DIFFICULTY\_DOMAIN} & CR Choice Understanding Difficulty: Domain-specific content in choices hard to grasp \\\\
\hline
\multicolumn{2}{|c|}{\textbf{Reading Comprehension (RC) - Verbal}} \\\\
\hline
\multicolumn{2}{|c|}{\textit{RC Reading Comprehension Errors}} \\\\
\texttt{RC\_READING\_COMPREHENSION\_ERROR\_VOCAB} & RC Reading Comprehension Error: Vocabulary misunderstanding \\\\
\texttt{RC\_READING\_COMPREHENSION\_ERROR\_LONG\_DIFFICULT\_SENTENCE\_ANALYSIS} & RC Reading Comprehension Error: Incorrect analysis of long/difficult sentences \\\\
\texttt{RC\_READING\_COMPREHENSION\_ERROR\_PASSAGE\_STRUCTURE} & RC Reading Comprehension Error: Misunderstanding of passage structure \\\\
\texttt{RC\_READING\_COMPREHENSION\_ERROR\_KEY\_INFO\_LOCATION\_UNDERSTANDING} & RC Reading Comprehension Error: Incorrect location or understanding of key information \\\\
\multicolumn{2}{|c|}{\textit{RC Question Understanding Errors}} \\\\
\texttt{RC\_QUESTION\_UNDERSTANDING\_ERROR\_FOCUS\_POINT} & RC Question Understanding Error: Misinterpretation of the question's focus point \\\\
\multicolumn{2}{|c|}{\textit{RC Location Skill Errors}} \\\\
\texttt{RC\_LOCATION\_SKILL\_ERROR\_LOCATION} & RC Location Skill Error: Incorrect location of information in the passage \\\\
\multicolumn{2}{|c|}{\textit{RC Reasoning Errors}} \\\\
\texttt{RC\_REASONING\_ERROR\_INFERENCE} & RC Reasoning Error: Flawed inference from the passage text \\\\
\multicolumn{2}{|c|}{\textit{RC Choice Analysis Errors}} \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_ERROR\_VOCAB} & RC Choice Analysis Error: Vocabulary issues in answer choices \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_ERROR\_SYNTAX} & RC Choice Analysis Error: Syntactical complexity in answer choices \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_ERROR\_LOGIC} & RC Choice Analysis Error: Logical structure of answer choices misunderstood \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_ERROR\_DOMAIN} & RC Choice Analysis Error: Domain-specific content in choices misunderstood \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_ERROR\_RELEVANCE\_JUDGEMENT} & RC Choice Analysis Error: Incorrect judgment of answer choice relevance \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_ERROR\_STRONG\_DISTRACTOR\_CONFUSION} & RC Choice Analysis Error: Confusion caused by strong distractor choices \\\\
\multicolumn{2}{|c|}{\textit{RC Method Errors}} \\\\
\texttt{RC\_METHOD\_ERROR\_SPECIFIC\_QUESTION\_TYPE\_HANDLING} & RC Method Error: Incorrect handling of specific question types \\\\
\multicolumn{2}{|c|}{\textit{RC Reading Comprehension Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{RC\_READING\_COMPREHENSION\_DIFFICULTY\_VOCAB\_BOTTLENECK} & RC Reading Comprehension Difficulty: Vocabulary bottleneck \\\\
\texttt{RC\_READING\_COMPREHENSION\_DIFFICULTY\_LONG\_DIFFICULT\_SENTENCE\_ANALYSIS} & RC Reading Comprehension Difficulty: Difficulty analyzing long/difficult sentences \\\\
\texttt{RC\_READING\_COMPREHENSION\_DIFFICULTY\_PASSAGE\_STRUCTURE\_GRASP\_UNCLEAR} & RC Reading Comprehension Difficulty: Unclear grasp of passage structure \\\\
\texttt{RC\_READING\_COMPREHENSION\_DIFFICULTY\_SPECIFIC\_DOMAIN\_BACKGROUND\_KNOWLEDGE\_LACK} & RC Reading Comprehension Difficulty: Lack of specific domain background knowledge \\\\
\multicolumn{2}{|c|}{\textit{RC Question Understanding Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{RC\_QUESTION\_UNDERSTANDING\_DIFFICULTY\_FOCUS\_POINT\_GRASP} & RC Question Understanding Difficulty: Difficulty grasping the question's focus point \\\\
\multicolumn{2}{|c|}{\textit{RC Location Skill Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{RC\_LOCATION\_SKILL\_DIFFICULTY\_INEFFICIENCY} & RC Location Skill Difficulty: Inefficient location of information \\\\
\multicolumn{2}{|c|}{\textit{RC Reasoning Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{RC\_REASONING\_DIFFICULTY\_INFERENCE\_SPEED\_SLOW} & RC Reasoning Difficulty: Slow inference speed \\\\
\multicolumn{2}{|c|}{\textit{RC Choice Analysis Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_DIFFICULTY\_VOCAB} & RC Choice Analysis Difficulty: Vocabulary challenges in answer choices \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_DIFFICULTY\_SYNTAX} & RC Choice Analysis Difficulty: Syntactical complexity in choices causing slowdown \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_DIFFICULTY\_LOGIC} & RC Choice Analysis Difficulty: Logical structure of choices hard to grasp \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_DIFFICULTY\_DOMAIN} & RC Choice Analysis Difficulty: Domain-specific content in choices hard to grasp \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_DIFFICULTY\_RELEVANCE\_JUDGEMENT} & RC Choice Analysis Difficulty: Difficulty judging answer choice relevance \\\\
\texttt{RC\_CHOICE\_ANALYSIS\_DIFFICULTY\_STRONG\_DISTRACTOR\_ANALYSIS} & RC Choice Analysis Difficulty: Difficulty analyzing strong distractor choices \\\\
\multicolumn{2}{|c|}{\textit{RC Method Difficulties (Efficiency Issue / Slow Performance)}} \\\\
\texttt{RC\_METHOD\_DIFFICULTY\_SPECIFIC\_QUESTION\_TYPE\_HANDLING} & RC Method Difficulty: Difficulty handling specific question types \\\\
\hline
...
\texttt{FOUNDATIONAL\_MASTERY\_APPLICATION\_INSTABILITY\_SFE} & General Foundational Mastery: Application instability (Special Focus Error) \\\\
\hline
\multicolumn{2}{|c|}{\textbf{Efficiency Bottlenecks (Q and DI Specific)}} \\\\ % Removed generic Verbal
\hline
...
\texttt{DI\_EFFICIENCY\_BOTTLENECK\_CALCULATION} & DI Calculation: Error in mathematical computation \\\\ % This was repeated, DI calc error is different from efficiency bottleneck
...
% Removed CR/RC excessive time tags as they are covered by "Difficulty" tags now
% Removed EFFICIENCY_BOTTLENECK_[AREA] for V
\hline
